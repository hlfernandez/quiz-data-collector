...
with open('quiz_responses.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Name', 'ID'] + [f'Question {i}' for i in range(1, num_questions+1)])
    writer.writerow([''] + true_answers)
    writer.writerows(responses)

print('Quiz responses saved to quiz_responses.csv')
