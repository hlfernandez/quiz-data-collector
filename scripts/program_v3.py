...
for i in range(1, num_questions+1):
    choices_str = inquirer.prompt([inquirer.Text(f'choices_{i}', message=f'What are the possible answer choices for question {i}? (e.g. A, B, C, D)')])[f'choices_{i}']
...
for i in range(1, num_questions+1):
    answer = None
    while answer not in possible_choices[i-1]:
        answer = inquirer.prompt([inquirer.Text(f'question_{i}', message=f'What is the correct answer for question {i}? ({", ".join(possible_choices[i-1])})')])[f'question_{i}']
...
while True:
    ...
    for i in range(1, num_questions+1):
        response = None
        while response not in possible_choices[i-1]:
            response = inquirer.prompt([inquirer.Text(f'question_{i}', message=f'What is your answer for question {i}? ({", ".join(possible_choices[i-1])})')])[f'question_{i}']
