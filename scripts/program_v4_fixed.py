...
choices_str = inquirer.prompt([inquirer.Text('choices', message='What are the possible answer choices? (e.g. A, B, C, D)')])['choices']
...
for i in range(1, num_questions+1):
    ...
    while answer not in possible_choices and answer != '':
        answer = inquirer.prompt([inquirer.Text(f'question_{i}', message=f'What is the correct answer for question {i}? ({", ".join(possible_choices)})')])[f'question_{i}']
...
while True:
    ...
    for i in range(1, num_questions+1):
        ...
        while response not in possible_choices and response != '':
            response = inquirer.prompt([inquirer.Text(f'question_{i}', message=f'What is your answer for question {i}? ({", ".join(possible_choices)})')])[f'question_{i}']
